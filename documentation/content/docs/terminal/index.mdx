...
```tsx title="Import"
import { useTerminalSize, useTerminalFocus, useShellOut } from 'giggles/terminal';
```

```tsx title="Shell Out Example"
import { useKeybindings, useFocus } from 'giggles';
import { useShellOut, useTerminalSize } from 'giggles/terminal';
...
function Editor({ path }: { path: string }) {
  ...
  useKeybindings(focus, {
    e: async () => {
      const { exitCode } = await shell.run(`${process.env.EDITOR ?? 'vim'} ${path}`);
      if (exitCode === 0) reload();
    },
  });

  return (
    <Box width={columns}>
      <Text>Press e to edit {path}</Text>
    </Box>
  );
}
```
