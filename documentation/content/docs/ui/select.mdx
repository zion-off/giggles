...
A controlled single-select list. Navigate with `j`/`k` or arrow keys, confirm with Enter.
...
<TypeTable
  type={{
    ...
    onChange: {
      description: 'Called with the new value when a selection is made. Fires on Enter by default, or on every highlight change when immediate is set.',
      type: '(value: T) => void',
      required: true,
    },
    onSubmit: {
