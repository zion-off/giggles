...
The example below shows `useKeybindingRegistry` used directly to build a `KeyHints` bar — the same registry that `CommandPalette` uses internally. Switch focus between panels with `j`/`k` to see the available commands update.
...
<TypeTable
  type={{
    ...
    maxVisible: {
      description: 'Maximum number of commands visible at once. When set, the list is windowed.',
      type: 'number',
    },
    paginatorStyle: {
      description: 'Style of the scroll indicator when the list is windowed',
