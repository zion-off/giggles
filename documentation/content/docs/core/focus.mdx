...
A collapsible file tree with two levels of navigation. The root group uses `j`/`k` to move between top-level entries. Each directory is its own `FocusGroup` — pressing `l` expands it and focus moves to the first file automatically via `useEffect`. Once inside, `j`/`k` on the inner group shadow the root group's bindings. Pressing `h` or `escape` collapses the directory, unmounting the inner group and automatically returning focus to the directory node so the root group's `j`/`k` resume. `useFocus` on the directory row stays green as long as any descendant has focus.

```tsx title="File Tree"
import { FocusGroup, useFocus, useFocusNode } from 'giggles';
...
function DirContent({ name, files, open, onClose }: { name: string; files: string[]; open: boolean; onClose: () => void }) {
  ...
  return (
    <Box flexDirection="column">
      ...
      {open && (
        <FocusGroup wrap={false} keybindings={({ next, prev }) => ({ j: next, k: prev, h: onClose, escape: onClose })}>
          {files.map((f, i) => <FileItem key={f} name={f} indent autoFocus={i === 0} />)}
        </FocusGroup>
      )}
    </Box>
  );
...
function DirItem({ name, files }: { name: string; files: string[] }) {
  ...
  return (
    <FocusGroup keybindings={open ? {} : { l: () => setOpen(true), enter: () => setOpen(true) }}>
