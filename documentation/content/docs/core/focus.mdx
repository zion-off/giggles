...
Scopes that use the same keys at different nesting levels don't conflict — the innermost scope that handles a key wins. When you want to yield control back to the parent (for example, to navigate between panels with the same j/k keys used inside them), call `escape()` from the inner scope's keybindings.

After `escape()`, the scope enters passive mode: `isPassive` becomes `true` and the scope is skipped during input dispatch. The parent scope's j/k now fire instead. Passive mode clears automatically when focus moves to a different scope.
...
function Panel({ title, items }: { title: string; items: string[] }) {
  const scope = useFocusScope({
    keybindings: ({ next, prev, escape }) => ({
      j: next,
      k: prev,
      h: escape  // yield — parent's j/k take over
    })
  });
...
```tsx title="File Tree"
import { FocusScope, useFocusNode, useFocusScope, useTheme } from 'giggles';
...
function FileTree() {
  const root = useFocusScope({
    keybindings: ({ nextShallow, prevShallow }) => ({
      j: nextShallow, k: prevShallow, down: nextShallow, up: prevShallow
    })
  });
