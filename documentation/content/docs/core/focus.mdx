...
Navigation between children is explicit: each scope exposes helpers (`next`, `prev`, `nextShallow`, `prevShallow`, `escape`, `drillIn`) directly on the returned handle, and also as the argument to the `keybindings` factory. This makes nested layouts composable — each scope only handles the keys you assign it.
...
<TypeTable
  type={{
    ...
    focusChildShallow: {
      description: 'Focus the direct child of this scope with the given focusKey, landing on its scope node without drilling in. Consistent with nextShallow. No-op if the key is not found.',
      type: '(key: string) => void',
      required: true,
    },
  }}
...
`Pick<FocusScopeHandle, 'next' | 'prev' | 'nextShallow' | 'prevShallow' | 'escape' | 'drillIn' | 'focusChild' | 'focusChildShallow'>` — the navigation methods passed as the argument to the `keybindings` factory.
