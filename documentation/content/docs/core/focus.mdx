...
Giggles maintains a focus tree — each call to `useFocus` registers a node, and `FocusGroup` organizes them into navigable groups. Only one node is focused at a time, and only components on the path from that node to the root receive key events.
...
<TypeTable
  type={{
    ...
    keybindings: {
      description: 'Keybindings active while any child of the group is focused',
      type: 'Keybindings',
    },
    children: {
