...
Registers keybindings that fire regardless of which component is focused. Use this for app-level shortcuts like quitting or opening help.

Global bindings are checked last — after the full focus path has been walked — so any focused component can always shadow them with its own binding for the same key.

```tsx title="Global shortcuts"
import { useGlobalKeybindings } from 'giggles';

function App() {
  useGlobalKeybindings({
    'ctrl+q': { action: () => process.exit(0), name: 'Quit' },
    '?': { action: openHelp, name: 'Help' },
  });
...
```ts title="Type Signature"
function useGlobalKeybindings(bindings: Keybindings): void
```
...
<TypeTable
  type={{
    ...
    available: {
      description: 'Named bindings currently reachable — focused/ancestor bindings from the active focus path, plus global bindings',
      type: 'RegisteredKeybinding[]',
      required: true,
    },
    local: {
...
<TypeTable
  type={{
    ...
    global: {
      description: 'True for bindings registered via useGlobalKeybindings — they fire as a fallback regardless of focus',
