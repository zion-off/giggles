...
// When searching, all unbound keys go to the fallback for text input
useKeybindings(
  focus,
  searchMode ? { enter: confirmSearch } : {},
  searchMode ? { fallback: handleSearchInput, bubble: ['escape', 'enter'] } : undefined
);
```
...
function FileManager() {
  ...
  // Search mode — fallback intercepts j/k for text input,
  // but escape/enter bubble to the named bindings above
  useKeybindings(
    focus,
...
function ContactForm() {
  ...
  // Edit mode — fallback intercepts j/k for text input,
  // but escape/enter bubble to the named bindings above
  useKeybindings(
    focus,
    editing
