...
A two-level tree where `j`/`k` move between top-level directories without entering them, and `l`/`enter` open the focused directory and move focus inside. The root uses [`nextShallow`/`prevShallow`](/core/focus#navigation-variants) to land on each directory node; the directory scope uses [`drillIn`](/core/focus#navigation-variants) to enter itself when opened, holding focus until the file list renders:
