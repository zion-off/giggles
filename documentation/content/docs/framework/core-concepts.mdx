...
2. **Capture mode** — if enabled and the key has no named binding, intercepts via `onKeypress`; keys in `passthrough` skip this and bubble to parent instead
...
6. **Global bindings** — registered with `useGlobalKeybindings`, checked last as a fallback

This priority order means:
- Capture mode truly captures — navigation keys become text input
- Named bindings fire before `onKeypress`, so special keys always work even in capture mode
