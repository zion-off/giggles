...
`useFocusNode()` generates a stable ID with React's `useId()`, then registers a node in the tree on mount and unregisters it on unmount:

```tsx
const focus = useFocusNode();
// focus.id — the node's ID
// focus.focused — whether this node or any descendant is currently focused
...
`useFocus()` is the companion hook for container components. Instead of registering a new node, it reads the `FocusNodeContext` ID that the nearest `FocusGroup` already published and returns a handle for it. `focused` is true when that group node or any of its descendants is focused — useful for borders and other visual indicators on wrapper components.
...
`FocusGroup` is a focus node that also scopes navigation for its children. It registers itself in the tree and publishes its own ID to `FocusNodeContext` so its children register under it.

Navigation is explicit: the `keybindings` prop accepts a function that receives `next`, `prev`, and `escape` helpers and returns a key map. `next` and `prev` call `navigateSibling`, which walks the sibling list and moves focus to the adjacent child — descending into the first leaf if the target has children of its own. `escape` focuses the group's own node, yielding control back to the parent group. No keys are bound automatically, so nested groups cannot shadow each other's bindings.
