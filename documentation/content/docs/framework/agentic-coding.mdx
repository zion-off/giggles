...
**Capture mode:** when `capture: true`, all keypresses are intercepted before checking named bindings, except keys listed in `passthrough`. Use this for text input. `TextInput` and `Autocomplete` use capture mode internally.

**`when: 'mounted'`:** registers a global binding active regardless of focus position. Useful for app-wide shortcuts.
