...
Developer experience utilities that build on the core framework and components.

## Progress

| Feature                        | Status      |
| ------------------------------ | ----------- |
| Logging & Debugging            | Not Started |
| Testing Utilities              | Not Started |
| Form State Management          | Not Started |
| Clipboard Integration          | Not Started |
| Mouse Event Integration        | Not Started |
| Background Tasks with Progress | Not Started |
...
```tsx
import { fireKey, render, waitFor } from 'giggles/testing';
...
<form.Field name="name">
  {({ value, error, onChange }) => <TextInput value={value} onChange={onChange} error={error} />}
</form.Field>;
```
